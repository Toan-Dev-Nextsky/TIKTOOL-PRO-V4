...
from tiktool_core import RebootTracker, backup_fingerprint  # noqa: E402
...
class PipelineTruthTests(unittest.TestCase):
    def test_batch_activate_fails_when_language_step_fails(self):
        """Catches a failed language command being reported as full success."""
        app = make_worker_app()
        app.var_set_lang_after_active = _Value(True)
        ...
        with patch.object(BB_RB, "which_tool", return_value="tool.exe"), patch.object(
            ...
            BB_RB,
            "run_capture",
            side_effect=[(0, "activated"), (0, "ok"), (9, "language failed")],
        ):
